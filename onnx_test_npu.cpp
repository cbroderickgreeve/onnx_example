...
int main(int argc, char* argv[])
{
    ...
    std::cout << "... Initialize session options" << std::endl;
    // initialize session options if needed
    OrtSessionOptions* session_options;
    CheckStatus(g_ort->CreateSessionOptions(&session_options));
    g_ort->SetIntraOpNumThreads(session_options, 1);

    std::cout << "... Set graph optimization level" << std::endl;
    // Sets graph optimization level
    g_ort->SetSessionGraphOptimizationLevel(session_options, ORT_ENABLE_BASIC);

    std::cout << "... Select execution provider" << std::endl;
    // Optionally add more execution provider via session options
    // E.g. for CUDA include cuda_provider_factory.h and uncomment the following line:
    // OrtSessionOptionsAppendExecutionProvider_CUDA(sessionOptions, 0);
    OrtSessionOptionsAppendExecutionProvider_VsiNpu(session_options, 0);

    std::cout << "Create session and load model into memory" << std::endl;
    //*************************************************************************
    ...
    OrtSession* session;
    #ifdef _WIN32
    const wchar_t* model_path = L"/usr/bin/squeezenet.onnx";
    #else
    const char* model_path = "/usr/bin/squeezenet.onnx";
    #endif

    printf("Using Onnxruntime C API\n");
    CheckStatus(g_ort->CreateSession(env, model_path, session_options, &session));

    std::cout << "... Get and print input layer info" << std::endl;
    //*************************************************************************
